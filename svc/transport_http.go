...
func MakeHTTPHandler(endpoints Endpoints, responseEncoder httptransport.EncodeResponseFunc, options ...httptransport.ServerOption) http.Handler {
	...
	m.Methods("GET").Path("/twitter/like_tweet/").Handler(httptransport.NewServer(
		endpoints.LikeTweetEndpoint,
		DecodeHTTPLikeTweetZeroRequest,
		responseEncoder,
		serverOptions...,
	))
	m.Methods("GET").Path("/twitter/like_tweet").Handler(httptransport.NewServer(
		endpoints.LikeTweetEndpoint,
		DecodeHTTPLikeTweetOneRequest,
		responseEncoder,
		serverOptions...,
	))
...
// DecodeHTTPLikeTweetZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded liketweet request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPLikeTweetZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.LikeTweetRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot read body of http request")
	}
...
// DecodeHTTPLikeTweetOneRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded liketweet request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPLikeTweetOneRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.LikeTweetRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
